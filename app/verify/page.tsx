...
export default function VerifyPage() {
  ...
  const handleVerify = () => {
    ...
    setResult(null);
    setTimeout(() => {
      setResult(registry[val] ? { id: val, ...registry[val] } : 'NOT_FOUND');
      setLoading(false);
    }, 1000);
  };
    ...
    <div style={{ padding: '40px', backgroundColor: '#050505', minHeight: '100vh', color: '#fff', display: 'flex', justifyContent: 'center' }}>
      <div style={{ width: '100%', maxWidth: '600px' }}>
        ...
        <div style={{ border: '1px solid #222', padding: '30px', backgroundColor: '#0a0a0a' }}>
          ...
          />
          <button onClick={handleVerify} style={{ width: '100%', marginTop: '20px', padding: '15px', background: '#D4AF37', border: 'none', fontWeight: 'bold', cursor: 'pointer', color: '#000' }}>
        ...
        {result && result !== 'NOT_FOUND' && (
          <div style={{ marginTop: '40px', border: '1px solid #D4AF37', padding: '40px', animation: 'fadeIn 0.5s' }}>
            {/* ပုံကို ဒီနေရာမှာ ခေါ်ပြထားပါတယ် */}
            <img src={`/${result.id}.jpg`} alt="Credential" style={{ width: '100%', marginBottom: '20px', border: '1px solid #333' }} />
